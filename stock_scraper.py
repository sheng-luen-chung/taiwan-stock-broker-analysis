...
class StockScraper:
    ...
    def download_stock_data(self, stock_code, max_retries=5):
        ...
        for attempt in range(max_retries):
            try:
                ...
                vcode = self.ocr.classification(img_bytes)
                self.log(f"OCR 識別結果: {vcode}")
                ...
                # 儲存為 UTF-8 with BOM 格式，讓 Excel 能正確識別中文
                ...
                self._save_processed_csv(csv_resp.text, processed_filename, stock_code)
    ...
    def _save_processed_csv(self, csv_content, filename, stock_code):
        """儲存處理後的 CSV 檔案，優化 Excel 相容性"""
        try:
            lines = csv_content.split("\n")
            
            # 使用 UTF-8 with BOM 儲存，確保 Excel 正確識別編碼
            ...
            self.log(f"處理後資料已儲存為: {filename}")
            
        except Exception as e:
            self.log(f"儲存處理後資料時發生錯誤: {str(e)}")
